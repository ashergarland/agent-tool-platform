...
const profileFor = (
  displayName: string,
  kind: ProfileKind,
): RegistryCapability['profiles'][number] => {
...
const entryFor = (
  ...
  return {
    ...
    version: {
      value: version,
      status: version === '0.0.0-development' ? 'development' : 'released',
    },
    artifacts: [
      {
        id: artifactId,
...
class FixtureRegistry implements CapabilityRegistryReader {
  public constructor(private readonly capabilities: readonly unknown[]) {}

  public getCapability(capabilityId: string): unknown {
    return this.capabilities.find(
      (entry) =>
        typeof entry === 'object' && entry !== null && 'id' in entry && entry.id === capabilityId,
    );
  }
}
...
describe('capability and binding resolution', () => {
  ...
  it('reports missing capabilities and exact versions', async () => {
    ...
    await expectAgentKitError(
      () => resolveRegistryCapability({ getCapability: () => null }, 'ast-summarizer'),
      'CAPABILITY_NOT_FOUND',
    );
  });
...
describe('deterministic build outputs', () => {
  ...
  it('keeps credentials, endpoints, provider IDs, and absolute paths out of agent.lock', async () => {
    const capabilities = [
      entryFor('vision', 'Vision', 'hybrid'),
      entryFor('azure', 'Azure', 'remote'),
    ];
    capabilities[1]!.profiles[0]!.prerequisites.requiredSecrets = [];
    capabilities[0]!.description = 'Boundary text containing synthetic-secret-value.';
    capabilities[0]!.routing.summary = 'Configured through https://private.internal/mcp.';
    capabilities[0]!.profiles[0]!.prerequisites.summary =
      'Prepared from C:\\Users\\operator\\private-state.json.';
    Object.assign(capabilities[0]!, { tenantId: '00000000-0000-0000-0000-000000000000' });
    ...
    expect(build.lockText).not.toContain('tenantId');
    expect(build.lockText).not.toContain('private.internal');
    expect(build.lockText).not.toContain('private-state.json');
