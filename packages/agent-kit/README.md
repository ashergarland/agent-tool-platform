...
The M5 package is private while the first-party registry package is landing in parallel. It is not
...
Agent Kit does not define or embed a catalog:

```ts
import type { CapabilityRegistryReader } from '@agent-tool-platform/agent-kit';

const registry: CapabilityRegistryReader = {
  getCapability(capabilityId) {
    return registryPackage.getCapability(capabilityId);
  },
};
```

Reader results are validated as the small build-time projection Agent Kit needs. The
six-dimensional profile summaries and explicit bindings are the normalized H1/H2 model derived
from capability declarations. Capability instructions and per-tool routing remain capability-owned
and are delivered by the MCP runtime; Agent Kit composes bounded capability/profile boundaries and
the registry routing summary rather than copying tool schemas. Artifacts, pinned source revisions,
and a digest of each consumed registry entry provide reproducible identity. Replacing the temporary
fixture reader with `@agent-tool-platform/capability-registry` does not change build semantics.
