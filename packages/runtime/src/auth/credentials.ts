...
/**
 * Credential comparison helpers, taken from the AST Summarizer implementation because it is the
 * strongest of the family: presented secrets are reduced to fixed-width keyed digests before
 ...
 * - The HMAC key is a fresh 32 random bytes per instance, so a digest cannot even be recomputed
...
const fingerprintDomain = 'agent-tool-platform:key-fingerprint:v1';
